...
export function PsuProductCard({ product }: { product: PsuProduct }) {
    ...
    >
      <div className="relative flex aspect-square items-center justify-center bg-slate-50 p-3">
        ...
        {product.imageUrl ? (
          ...
          <BsPlug size={56} className="text-slate-300" aria-hidden="true" />
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-3">
        ...
        <p className="text-xs text-slate-400">
          {product.wattage} | {product.modular}
        </p>
        <p className="text-[11px] text-slate-400">
          {product.certification} · {product.brand}
        </p>
