...
import {
  ...
  motherboardChipsetOptions,
  motherboardCpuSupportOptions,
  motherboardFormFactorOptions,
  motherboardMaxMemoryOptions,
  motherboardMemorySlotOptions,
  motherboardMemoryTypeOptions,
  motherboardSocketOptions,
...
interface MotherboardFilterSidebarProps {
  title: string;
  count: number;
  sort: CategorySort;
  ...
  onChange: (next: MotherboardFilterState) => void;
  onClear: () => void;
}

export function MotherboardFilterSidebar({
  title,
  count,
  sort,
  onSortChange,
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-4 lg:sticky lg:top-4 lg:w-72">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <CollapsibleSection title="CPU Support">
          {motherboardCpuSupportOptions.map((cpuSupport) => (
            <Checkbox
              key={cpuSupport}
              label={cpuSupport}
              checked={filters.cpuSupports.includes(cpuSupport)}
              onChange={() => onChange({ ...filters, cpuSupports: toggle(filters.cpuSupports, cpuSupport) })}
            />
          ))}
        </CollapsibleSection>

        <CollapsibleSection title="CPU Socket">
          {motherboardSocketOptions.map((socket) => (
            <Checkbox
              key={socket}
              label={socket}
        ...
        <CollapsibleSection title="Chipset">
          {motherboardChipsetOptions.map((chipset) => (
            <Checkbox
              key={chipset}
              label={chipset}
              checked={filters.chipsets.includes(chipset)}
              onChange={() => onChange({ ...filters, chipsets: toggle(filters.chipsets, chipset) })}
            />
          ))}
        </CollapsibleSection>

        <CollapsibleSection title="Mainboard Support">
          {motherboardFormFactorOptions.map((formFactor) => (
            <Checkbox
              key={formFactor}
              label={formFactor}
              checked={filters.formFactors.includes(formFactor)}
              onChange={() => onChange({ ...filters, formFactors: toggle(filters.formFactors, formFactor) })}
            />
          ))}
        </CollapsibleSection>

        <CollapsibleSection title="Memory Slots">
          {motherboardMemorySlotOptions.map((slot) => (
            <Checkbox
              key={slot}
              label={slot}
              checked={filters.memorySlots.includes(slot)}
              onChange={() => onChange({ ...filters, memorySlots: toggle(filters.memorySlots, slot) })}
            />
          ))}
        </CollapsibleSection>

        <CollapsibleSection title="Memory Type">
          {motherboardMemoryTypeOptions.map((memoryType) => (
            <Checkbox
              key={memoryType}
              label={memoryType}
              checked={filters.memoryTypes.includes(memoryType)}
              onChange={() => onChange({ ...filters, memoryTypes: toggle(filters.memoryTypes, memoryType) })}
            />
          ))}
        </CollapsibleSection>

        <CollapsibleSection title="Max Memory">
          {motherboardMaxMemoryOptions.map((maxMemory) => (
            <Checkbox
              key={maxMemory}
              label={maxMemory}
              checked={filters.maxMemories.includes(maxMemory)}
              onChange={() => onChange({ ...filters, maxMemories: toggle(filters.maxMemories, maxMemory) })}
            />
          ))}
        </CollapsibleSection>
