...
import { psuBrandOptions, psuWattageOptions, type PsuFilterState } from '../../data/psuProducts';
...
interface PsuFilterSidebarProps {
  title: string;
  count: number;
  sort: CategorySort;
  ...
  priceCeiling: number;
  filters: PsuFilterState;
  onChange: (next: PsuFilterState) => void;
  onClear: () => void;
}

export function PsuFilterSidebar({
  title,
  count,
  sort,
  onSortChange,
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-4 lg:w-72">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <CollapsibleSection title="Continuous Power W">
          {psuWattageOptions.map((wattage) => (
            <Checkbox
              key={wattage}
              label={wattage}
              checked={filters.wattages.includes(wattage)}
              onChange={() => onChange({ ...filters, wattages: toggle(filters.wattages, wattage) })}
            />
          ))}
        </CollapsibleSection>
