...
const badgeStyle: Record<MotherboardBadge, string> = {
  แนะนำ: 'bg-ink text-white',
  สินค้าขายดี: 'bg-brand text-white',
  ใหม่: 'bg-amber-400 text-ink',
};

export function MotherboardProductCard({ product }: { product: MotherboardProduct }) {
    ...
    >
      <div className="relative flex aspect-square items-center justify-center bg-slate-50 p-3">
        ...
        {product.imageUrl ? (
          ...
          <BsMotherboard size={56} className="text-slate-300" aria-hidden="true" />
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-3">
        ...
        <p className="text-xs text-slate-400">
          {product.chipset} | {product.memoryType}
        </p>
