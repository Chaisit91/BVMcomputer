...
import { BsDeviceSsd, BsHdd } from 'react-icons/bs';
import { FiShoppingCart, FiZap } from 'react-icons/fi';
import { Link } from 'react-router-dom';
import { useAppDispatch } from '../../app/hooks';
import { addToCart } from '../../features/cart/cartSlice';
import type { StorageBadge, StorageProduct } from '../../data/storageProducts';
import { formatTHB } from '../../lib/format';
import { FavoriteButton } from '../ui/FavoriteButton';
import { SpecChip } from '../ui/SpecChip';

const badgeStyle: Record<StorageBadge, string> = {
...
export function StorageProductListRow({ product }: { product: StorageProduct }) {
    ...
    >
      ...
      <div className="relative flex h-28 w-28 shrink-0 items-center justify-center rounded-lg bg-slate-50 p-2">
        ...
        {product.imageUrl ? (
          ...
          <BsDeviceSsd size={44} className="text-slate-300" aria-hidden="true" />
        )}
      </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2.5 flex flex-wrap gap-1.5">
          <SpecChip icon={BsHdd} label={product.capacity} />
          <SpecChip icon={FiZap} label={product.readSpeed} />
          <SpecChip icon={BsDeviceSsd} label={product.type} />
        </div>
      </div>
