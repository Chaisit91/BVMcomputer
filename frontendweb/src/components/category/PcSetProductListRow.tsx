...
import { BsCpu, BsGpuCard, BsMemory, BsPcDisplay } from 'react-icons/bs';
import { FiShoppingCart } from 'react-icons/fi';
import { Link } from 'react-router-dom';
import { useAppDispatch } from '../../app/hooks';
import { addToCart } from '../../features/cart/cartSlice';
import type { PcSetBadge, PcSetProduct } from '../../data/pcSetProducts';
import { FavoriteButton } from '../ui/FavoriteButton';
import { PriceTag } from '../ui/PriceTag';
import { SpecChip } from '../ui/SpecChip';

const badgeStyle: Record<PcSetBadge, string> = {
...
export function PcSetProductListRow({ product }: { product: PcSetProduct }) {
    ...
    >
      ...
      <div className="relative flex h-28 w-28 shrink-0 items-center justify-center rounded-lg bg-slate-50 p-2">
        ...
        {product.imageUrl ? (
          ...
          <BsPcDisplay size={44} className="text-slate-300" aria-hidden="true" />
        )}
      </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-xs text-slate-400">
          {product.brand} | {product.usageType}
        </p>
        <div className="mt-2.5 flex flex-wrap gap-1.5">
          <SpecChip icon={BsCpu} label={product.cpu} />
          <SpecChip icon={BsGpuCard} label={product.gpu} />
          <SpecChip icon={BsMemory} label={product.ram} />
        </div>
      </div>
