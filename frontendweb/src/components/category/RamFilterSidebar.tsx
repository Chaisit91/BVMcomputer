...
interface RamFilterSidebarProps {
  title: string;
  count: number;
  sort: CategorySort;
  ...
  priceCeiling: number;
  filters: RamFilterState;
  onChange: (next: RamFilterState) => void;
  onClear: () => void;
}

export function RamFilterSidebar({
  title,
  count,
  sort,
  onSortChange,
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-4 lg:w-72">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <CollapsibleSection title="Memory Series">
          {ramSeriesOptions.map((series) => (
            <Checkbox
              key={series}
              label={series}
        ...
        <CollapsibleSection title="Memory Capacity">
          {ramCapacityOptions.map((capacity) => (
            <Checkbox
              key={capacity}
              label={capacity}
              checked={filters.capacities.includes(capacity)}
              onChange={() => onChange({ ...filters, capacities: toggle(filters.capacities, capacity) })}
            />
          ))}
        </CollapsibleSection>
        ...
        <CollapsibleSection title="Speed">
          {ramSpeedOptions.map((speed) => (
            <Checkbox
              ...
              onChange={() => onChange({ ...filters, speeds: toggle(filters.speeds, speed) })}
            />
          ))}
        </CollapsibleSection>

        <CollapsibleSection title="Memory Type">
          {ramMemoryTypeOptions.map((memoryType) => (
            <Checkbox
              key={memoryType}
              label={memoryType}
