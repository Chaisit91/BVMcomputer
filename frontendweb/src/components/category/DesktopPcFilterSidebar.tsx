...
import {
  desktopPcBrandOptions,
  desktopPcCpuPlatformOptions,
  desktopPcUsageTypeOptions,
...
interface DesktopPcFilterSidebarProps {
  title: string;
  count: number;
  sort: CategorySort;
  ...
  onChange: (next: DesktopPcFilterState) => void;
  onClear: () => void;
}

export function DesktopPcFilterSidebar({
  title,
  count,
  sort,
  onSortChange,
    ...
    <aside className="flex w-full shrink-0 flex-col gap-4 lg:sticky lg:top-4 lg:w-72">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <CollapsibleSection title="Brand" defaultOpen>
          {desktopPcBrandOptions.map((brand) => (
            <Checkbox
              key={brand}
              label={brand}
        ...
        <CollapsibleSection title="Usage Type">
          {desktopPcUsageTypeOptions.map((usageType) => (
            <Checkbox
              key={usageType}
              label={usageType}
              checked={filters.usageTypes.includes(usageType)}
              onChange={() => onChange({ ...filters, usageTypes: toggle(filters.usageTypes, usageType) })}
            />
          ))}
        </CollapsibleSection>

        <CollapsibleSection title="CPU Platform">
          {desktopPcCpuPlatformOptions.map((cpuPlatform) => (
            <Checkbox
              key={cpuPlatform}
              label={cpuPlatform}
              checked={filters.cpuPlatforms.includes(cpuPlatform)}
              onChange={() => onChange({ ...filters, cpuPlatforms: toggle(filters.cpuPlatforms, cpuPlatform) })}
            />
          ))}
        </CollapsibleSection>
