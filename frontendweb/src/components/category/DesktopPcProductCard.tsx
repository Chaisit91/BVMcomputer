...
const badgeStyle: Record<DesktopPcBadge, string> = {
  แนะนำ: 'bg-ink text-white',
  สินค้าขายดี: 'bg-brand text-white',
  ใหม่: 'bg-amber-400 text-ink',
};

export function DesktopPcProductCard({ product }: { product: DesktopPcProduct }) {
    ...
    >
      <div className="relative flex aspect-square items-center justify-center bg-slate-50 p-3">
        ...
        {product.imageUrl ? (
          ...
          <PiComputerTower size={56} className="text-slate-300" aria-hidden="true" />
        )}
      </div>
