...
import {
  pcSetBrandOptions,
  pcSetCpuPlatformOptions,
  pcSetUsageTypeOptions,
...
interface PcSetFilterSidebarProps {
  title: string;
  count: number;
  sort: CategorySort;
  ...
  priceCeiling: number;
  filters: PcSetFilterState;
  onChange: (next: PcSetFilterState) => void;
  onClear: () => void;
}

export function PcSetFilterSidebar({
  title,
  count,
  sort,
  onSortChange,
    ...
    <aside className="flex w-full shrink-0 flex-col gap-4 lg:sticky lg:top-4 lg:w-72">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <CollapsibleSection title="Usage Type">
          {pcSetUsageTypeOptions.map((usageType) => (
            <Checkbox
              key={usageType}
              label={usageType}
        ...
        <CollapsibleSection title="CPU Platform">
          {pcSetCpuPlatformOptions.map((cpuPlatform) => (
            <Checkbox
              key={cpuPlatform}
              label={cpuPlatform}
