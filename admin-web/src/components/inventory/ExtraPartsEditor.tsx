...
export function ExtraPartsEditor({ parts, onChange }: ExtraPartsEditorProps) {
  const addPart = () => {
      ...
      {parts.map((part) => (
        <div key={part.id} className="flex items-center gap-3">
          <input
            ...
            className="w-40 shrink-0 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-900 outline-none placeholder:text-gray-400 focus:border-rose-400"
          />
          <input
            type="text"
            value={part.value}
