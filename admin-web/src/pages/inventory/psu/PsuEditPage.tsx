...
export function PsuEditPage({ readOnly = false }: { readOnly?: boolean }) {
  const { psuId = '' } = useParams()
  const navigate = useNavigate()
  const currentUser = useAppSelector((state) => state.auth.user)
  const [status, setStatus] = useState<LoadStatus>('loading')
  const [detail, setDetail] = useState<Psu | null>(null)
  ...
  const onSubmit = handleSubmit(async (values) => {
    await savePsu(psuId, { ...values, videoLinks, extraSpecs })
  ...
  if (status === 'not_found') {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-3 text-sm text-gray-400">
        <p>ไม่พบพาวเวอร์ซัพพลายที่ต้องการ</p>
        <Link to="/inventory/psu" className="text-rose-500 hover:underline">
          กลับไปหน้ารายการ
        </Link>
      </div>
    )
  ...
  const pageTitle = readOnly ? 'ดูพาวเวอร์ซัพพลาย (View Power Supply)' : 'แก้ไขข้อมูล พาวเวอร์ซัพพลาย (Edit Power Supply)'
